...
class Admin extends CI_Controller {
	...
	{
		$query=$this->User_model->get_where(["level" => 1]);
		$res = $query->result();
		...
		if ($num_rows > 0)
		{
			...
			foreach ($res as $row){
				...
				$this->table->add_row(	++$i,
							...
							$row->username,
							$sts,
							anchor('admin/ubah/'.e_url($row->id_user),'<span class="fa fa-pencil-alt"></span>',array( 'title' => 'Ubah', 'class' => 'btn btn-primary btn-xs', 'data-toggle' => 'tooltip')).'&nbsp;'.
							$btn
						);
			}
		}
	...
	public function index()
	{
		$data = array(
						"page" => "admin_view",
						"ket" => "Data",
						"add" => anchor('admin/tambah', '<i class="fa fa-plus"></i>', array("class" => "btn btn-success", "data-toggle" => "tooltip", "data-placement" => "top", "title" => "Tambah Data")),
	...
	{
		$q = $this->User_model->get_where_jabatan(["level" => 1]);
		$res = $q->result();
